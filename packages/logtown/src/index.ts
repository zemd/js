...
>;
export interface LoggerWrapper extends Partial<
  Record<LoggerWrapperFnName, WrapperLoggerFn> & Record<string, any>
> {}
...
function getStatus(id: string, level: LogLevel): ModuleLogLevelStatus {
  ...
  const moduleLevelRule = ((globalThis as any)[LOGTOWN_RULES_SYMBOL] as LogRuleStorage)
    .get(safeId)
    ?.get(level);
  const moduleWildcardRule = ((globalThis as any)[LOGTOWN_RULES_SYMBOL] as LogRuleStorage)
    .get(safeId)
    ?.get("*");
  const globalLevelRule = ((globalThis as any)[LOGTOWN_RULES_SYMBOL] as LogRuleStorage)
    .get("*")
    ?.get(level);
  ...
  return (
    globalLevelRule ?? globalWildcardRule ?? moduleLevelRule ?? moduleWildcardRule ?? "enabled"
  );
}
