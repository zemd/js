...
const renderCombinedReleaseBody = ({ published, paths, npmState = "published", notes }) => {
	const out = [];
	...
	out.push("| :--- | ---: |");
	for (const { name, version } of published) out.push(`| [\`${name}\`](https://www.npmjs.com/package/${name}) | \`${version}\` |`);
	out.push("");
	out.push("### Changelogs");
	out.push("");
...
const releasePublishedPackages = async ({ api, sha, published, workspace, npmState = "published", now = /* @__PURE__ */ new Date() }) => {
...
const githubReleases = {
	...
	run: async (argv) => {
		...
		if (!summaryPath || !workspacePath) throw new Error("usage: github-releases <publish-summary.json> <workspace-list.json> [published|staged]");
		if (rawState !== "published" && rawState !== "staged") throw new Error(`github-releases: expected npm state "published" or "staged", got "${rawState}"`);
		const npmState = rawState;
