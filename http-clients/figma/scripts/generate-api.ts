    ...
    declarations: [
      {
        ...
        initializer: (writer) => {
          ...
          writer
            .write(
              `const endpoint = createEndpoint([prefix("${apiServerUrl}"), json(), ...initialTransformers]);`,
            )
            .newLine();
              ...
              writer.block(() => {
                ...
                for (const namespace of versionNamespaces) {
                  ...
                  for (const operation of operationsFiltered) {
                    ...
                    const queryIsOptional: boolean =
                      !!operation.queryParams && !operation.queryParams.required;
                    ...
                    writer.block(() => {
                      if (queryIsOptional) {
                        ...
                        writer
                          .write(`if (options) { transformers.push(query(options)); }`)
                          .indent();
...
async function main() {
  ...
  await writeFile(
    resolve(__dirname, "..", "src", "openapi.json"),
